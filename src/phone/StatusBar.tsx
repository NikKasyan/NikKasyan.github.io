...
export const StatusBar: Component<Props> = (props) => {
    const { timeInMinutes: time, batteryPercentage: battery, network, wifi } = props;
    return (
        <div class="statusbar">
            <span>{formatTime(time)}</span>
            <div class="info">
                <Show when={wifi !== undefined}>
                    <WifiIndicator wifi={wifi!} />
                </Show>
                <Show when={network !== undefined}>
                    <NetworkSignal network={network!} wifiEnabled={wifi?.enabled ?? false} />
                </Show>
