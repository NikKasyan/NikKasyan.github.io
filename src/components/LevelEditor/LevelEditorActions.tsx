...
export const LevelEditorActions = () => {
    ...
    return (
        <div class="level-editor-actions">
            <button onClick={() => (resetEditorLevel(), updateLevel(createDefaultLevel()), navigate("/editor"))}>Clear Level</button>
            ...
            <input style={{ display: "none" }} ref={setFileInput} type="file" id="fileInput" accept=".json" onChange={(e) => {
               ...
               importLevel(file).then(level=> (resetEditorLevel(), updateLevel(createDefaultLevel()), level)).then(level => updateLevel(level)).then(() => navigate("/editor"))
            }} />
            <A href="/editor"><button>Overview</button></A>
        </div>
    )
};
